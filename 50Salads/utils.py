...
def import_pre_extracted_50Salads(sample, frames, modality, mode):
    """
    Input:
        frames: a list of frames
        modality: obj, gaze, hands, rgb, flow
    ...
    path_to_feats= '/50salads/features/'
    ...
    data_npy = np.load(path_to_feats + '/' + actor + '.npy')
...
def Add_Noise_Action_ShortLong(word_em, percentage, shortlong):
    ...
    if len(get_indexes)>1:
        ...
        # recognizer percentange
        correct_class_prob = 76.5# HERE add the percentage of correct classification
...
#                       
                        ...
                        if Irecon > correct_class_prob: #Add noise
                            ...
                            # Get a random new label (verb and noun pair)
                            idx_new_lbl = random.randint(0,21)
                            while idx_new_lbl in [1,13,18,21]:
                                idx_new_lbl = random.randint(0, 21)

                            if idx_new_lbl in [1,13,18]:
                                print(idx_new_lbl)
                                raise ValueError('You are planning to add an index that corresponds to a) start, b) end, c) ''')
...
def calculate_timesteps(video_id, end_frame, mode, dataset="50Salads"):
    """
       Input:
    ...
    if mode == 'train':
        path = path + '/Salads/50salads_anticipation_sorted_train.split1.bundle.csv'
    elif mode == 'val':
        raise ValueError('There is no validation in 50 Salads')
        path = path + '/val.csv'
    else:
