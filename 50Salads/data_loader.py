...
from utils import import_pre_extracted, calculate_timesteps, import_pre_extracted_Assembly, Add_Noise, Add_Noise_Action, Add_Noise_Action_ShortLong, Add_Noise_ShortLong,import_pre_extracted_50Salads
...
class DataSet():
    ...
    def get_words_ActionPrediction(self):
        ...
        if self.TrainValTest == 'train':
            path = path + '/Salads/50salads_anticipation_sorted_train.split1.bundle.txt'
   ...
   ###################################################################################################
    ...
    def get_words_ActionPrediction_Action(self):
        ...
        if self.TrainValTest == 'train':
            ...
            path = os.path.join('splits')
            if self.TrainValTest == 'val':
                raise ValueError('Salads has no validation set!')
            else:
              #  raise ValueError('Need to add the path to the test txt')
                path = path + '/Salads/50salads_anticipation_sorted_test.split1.bundle.txt'
        ...
        for i in range(len(input_sequences)):
            if (input_sequences[i, -2] == 0 or input_sequences[i, -1] == 0):
               ...
               # in_seq = Add_Noise_Action(input_sequences[i, :-1], percentage)
                ...
                if input_sequences[i, -1] == 13 or input_sequences[i, -1] == 18:
                    print(input_sequences[i])
                    print(input_sequences[i, -1] )
                    raise ValueError('Should not have 1, 13, 18')
    ...
    def get_words(self):
        ...
        if self.TrainValTest == 'train':
            raise ValueError('What are you doing here')
    ...
    def get_data(self):
        ...
        if self.TrainValTest == 'train':
        # ---- Full action ---
         #path = path + '/CVRL_anticipation_train_validation_test_files/train-validation_challenge/fullaction/Set_08_02_2023_assembly101_anticipation_train_e4_action_cls_sorted.csv'
        # ----- Verb or Object -----------
           path = path + '/Salads/50salads_anticipation_sorted_train.split5.bundle.csv'
        elif self.TrainValTest == 'val':
            # Full Action
          #  path = path + '/CVRL_anticipation_train_validation_test_files/train-validation_challenge/fullaction/Set_08_02_2023_assembly101_anticipation_validation_e4_action_cls_sorted.csv'
            # ----- Verb or Object -----
            raise ValueError('Salads has no validation set')
         ...
         #   path = path + '/CVRL_anticipation_train_validation_test_files/train-validation_challenge/fullaction/Set_08_02_2023_assembly101_anticipation_validation_e4_action_cls_sorted.csv'
            # ---- Verb or Object
            path = path + '/Salads/50salads_anticipation_sorted_test.split1.bundle.csv'
        #
    ...
    def get_frames_for_sample(self, sample, RandomClip=False, seq_len=8, data_type='images', feat_mod='obj', timesteps = True):
        ...
        if self.model_name not in ['TSN_AssemblyOnly', 'TSN_AssemblyNLP']:
            raise ValueError('Does not support anything else!')
        ...
        if timesteps:
          ...
          if data_type in ['images']:
              ...
              if self.model_name in ['TSN_AssemblyOnly', 'TSN_AssemblyNLP']:
                  ...
                  feat_left, feat_right, feat_diff = import_pre_extracted_50Salads(sample, images, feat_mod, mode_ext)
