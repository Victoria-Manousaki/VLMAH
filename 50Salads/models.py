...
class ResearchModels():
    ...
    lossweights = False, saved_model=None,weights=None, Custom_cros=False, Loss_req=None):
        ...
        elif model in ['TSN_AssemblyOnly']:
            print('Setting up a model with RGB TSN input!')
            self.model = self.TSN_AssemblyOnly()
        elif model in ['TSN_AssemblyNLP']:
            print('Setting up a model with RGB TSN and NLP input!')
            self.model = self.TSN_AssemblyNLP()
    ...
    def NLP_Video(self):
        ...
        #model_visual_A = Dropout(0.5)(model_visual)
...
#                ASSEMBLY 101
###################################################################################################################\
    def TSN_AssemblyOnly(self):

        inputsTSNLeft = Input(shape=(self.Frame_seq_len, 1024))

        inputsTSNRight = Input(shape=(self.Frame_seq_len, 1024))

        inputsDiff = Input(shape=(self.Frame_seq_len, 1024))

        model_objs = Bidirectional(LSTM(256,return_sequences=True), batch_input_shape=( None, self.Frame_seq_len,1024))(inputsTSNLeft)

        model_gaze = Bidirectional(LSTM(256, return_sequences=True), batch_input_shape=(None, self.Frame_seq_len, 1024))(inputsTSNRight)

        model_hands = Bidirectional(LSTM(256, return_sequences=True), batch_input_shape=(None, self.Frame_seq_len, 1024))(inputsDiff)
        ...
        model = Model(inputs=[inputsTSNLeft, inputsTSNRight, inputsDiff], outputs=model_works, name="TSN_AssemblyOnly")
        return model

    def TSN_AssemblyNLP(self):
        ...
        model = Model(inputs= [inputsWords, inputsTSNLeft, inputsTSNRight, inputsDiff], outputs= model_pout,name="TSN_AssemblyNLP")
