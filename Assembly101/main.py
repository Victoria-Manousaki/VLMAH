...
def trainANDtest(data_type, modeTrainTest = 'train', modelName=None, saved_model=None, PoS_focus = 'noun', image_shape=None, batch_size=1,
    ...
    if modeTrainTest is 'test':
      ...
      data = DataSet(seq_length=frame_seq_len, data_type=data_type, sampling_type=0, TrainValTest='test', model_name=modelName)
      ...
      score = model.model.evaluate_generator(val_generator,steps=steps_todo-1, use_multiprocessing=False,verbose=1)
      ...
      np.save('y_pred.npy',np.array(y_prediction))
        ...
        if modeTrainTest is 'retrain':
            ...
            model = load_model(path2save+'\model_'+modelName+'_'+str(epoch_last)+'.h5')
            ...
            for ep in range(nb_epoch):
                ...
                hist = model.fit_generator(
                    ...
                    #  callbacks=[LearningRateReducerCb()],
                      ...
                      workers=2,  initial_epoch = ep)
                
                model.save(path2save+'/model_'+modelName+'_'+str(ep+int(epoch_last)+1)+'.h5')
            ...
            if Multi_GPU:
              ...
              for ep in range(nb_epoch):
                  ...
                  hist = model.model.fit_generator(
                        ...
                        verbose=1,
                        use_multiprocessing=False,
                        workers=2,  initial_epoch = ep, class_weight= weights)
...
def main():
    ...
    batch_size = 4
    Trained = 'train' # train or test or retrain

    epoch_last = input('Epoch checkpoint:') if Trained in ['retrain','test'] else None
    ...
    model = 'TSN_AssemblyNLP' #'NLP' # Vid #VNLP #Obj_Only # Obj_NLP # ObjsGazeHands_Only, # ObjsGazeHands_NLP # VNLP_Both, # ObjsGazeTSNRGB_NLP # TSN_AssemblyOnly, # TSN_AssemblyNLP
